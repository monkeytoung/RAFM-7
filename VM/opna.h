...
// ---------------------------------------------------------------------------
//	$Id: opna.h,v 1.18 2000/09/08 13:45:57 cisc Exp $
...
namespace FM
{
	...
	public:
		...
		bool FASTCALL	Init(uint c, uint rf, uint rp);
		virtual void FASTCALL Reset();

		void FASTCALL	SetVolumeFM(int db);
		void FASTCALL	SetVolumePSG(int db);

		PSG		psg;
	protected:
		void FASTCALL	SetParameter(Channel4* ch, uint addr, uint data);
		void FASTCALL	SetPrescaler(uint p);
		void FASTCALL	RebuildTimeTable();
	...
	protected:
		...
		void FASTCALL	SetStatus(uint bit);
		void FASTCALL	ResetStatus(uint bit);
		void	UpdateStatus();
		void	LFO();
	...
	public:
		...
		bool FASTCALL	Init(uint c, uint r, bool=false, const char* =0);
		bool FASTCALL	SetRate(uint c, uint r, bool);

		void FASTCALL	Reset();
		void FASTCALL 	Mix(Sample* buffer, int nsamples);
		void FASTCALL 	SetReg(uint addr, uint data);
		uint FASTCALL	GetReg(uint addr);
		uint FASTCALL	ReadStatus() { return status & 0x03; }
		uint FASTCALL	ReadStatusEx() { return 0xff; }

		void FASTCALL	SetChannelMask(uint mask);
...
inline void FASTCALL FM::OPNBase::RebuildTimeTable()
{
	int p = prescale;
	prescale = -1;
	SetPrescaler(p);
...
inline void FASTCALL FM::OPNBase::SetVolumePSG(int db)
{
	psg.SetVolume(db);
}
