...
// ---------------------------------------------------------------------------
//	$Id: opna.cpp,v 1.39 2000/09/08 13:45:57 cisc Exp $
...
// ---------------------------------------------------------------------------
//	2ch 分の計算を並列して行う asm 版コアを使用する
//	若干の速度向上がみられる？
//	
//	注意: asm 版コアを使用しないときは却って性能が低下するので
//		  コメントアウトすること．
//		・OPNB(4ch 版) として使用する場合は使わない方が速いかも．
//
//#define FM_USE_CALC2
...
//	パラメータセット
void FASTCALL OPNBase::SetParameter(Channel4* ch, uint addr, uint data)
{
...
//	プリスケーラ設定
void FASTCALL OPNBase::SetPrescaler(uint p)
{
	...
	if (prescale != p)
	{
		...
		// (1+FM_LFOCBITS) / table2[i]
	}
}

//	初期化
bool FASTCALL OPNBase::Init(uint c, uint rf, uint rp)
{
	clock = c;
	rate = rf;
	psgrate = rp;
...
//	音量設定
void FASTCALL OPNBase::SetVolumeFM(int db)
{
	db = Min(db, 20);
	if (db > -192)
...
//	初期化
bool FASTCALL OPN::Init(uint c, uint r, bool ip, const char*)
{
	if (!SetRate(c, r, ip))
		return false;
...
//	サンプリングレート変更
bool FASTCALL OPN::SetRate(uint c, uint r, bool ip)
{
	interpolation = ip;
...
//	レジスタ読み込み
uint FASTCALL OPN::GetReg(uint addr)
{
	if (addr < 0x10)
...
//	レジスタアレイにデータを設定
void FASTCALL OPN::SetReg(uint addr, uint data)
{
//	LOG2("reg[%.2x] <- %.2x\n", addr, data);
	if (addr >= 0x100)
		return;
...
//	ステータスフラグ設定
void FASTCALL OPN::SetStatus(uint bits)
{
	if (!(status & bits))
	{
		status |= bits;
...
void FASTCALL OPN::ResetStatus(uint bit)
{
	status &= ~bit;
	if (!status)
		Intr(false);
...
//	マスク設定
void FASTCALL OPN::SetChannelMask(uint mask)
{
	for (int i=0; i<3; i++)
		ch[i].Mute(!!(mask & (1 << i)));
...
//	合成(1ch)
void FASTCALL OPN::Mix(Sample* buffer, int nsamples)
{
	ISample x, y, z;
	int i;
		...
		if (interpolation)
		{
			...
			for (; nsamples > 0; nsamples--)
			{
				while (mixdelta > 0)
				{
					...
					if (actch & 0x14) Channel4::Calc2(&ch[1], &s, &s);
#else
...
void OPNABase::MixSubSL(int activech, ISample** dest)
{
	...
	if (--fbch > 0)
	{
		if (activech & 0x005) Channel4::CalcL2E(&ch[0], dest[0], dest[1]);
		if (activech & 0x050) Channel4::CalcL2E(&ch[2], dest[2], dest[3]);
		if (activech & 0x500) Channel4::CalcL2E(&ch[4], dest[4], dest[5]);
	}
	else
	{
		fbch = 2;
		if (activech & 0x005) Channel4::CalcL2(&ch[0], dest[0], dest[1]);
		if (activech & 0x050) Channel4::CalcL2(&ch[2], dest[2], dest[3]);
		if (activech & 0x500) Channel4::CalcL2(&ch[4], dest[4], dest[5]);
	}
#elif defined(FM_USE_CALC2)
	if (activech & 0x005) Channel4::CalcL2(&ch[0], dest[0], dest[1]);
	if (activech & 0x050) Channel4::CalcL2(&ch[2], dest[2], dest[3]);
	if (activech & 0x500) Channel4::CalcL2(&ch[4], dest[4], dest[5]);
#else
...
inline void OPNABase::MixSubS(int activech, ISample** dest)
{
	...
	if (--fbch > 0)
	{
		if (activech & 0x005) Channel4::Calc2E(&ch[0], dest[0], dest[1]);
		if (activech & 0x050) Channel4::Calc2E(&ch[2], dest[2], dest[3]);
		if (activech & 0x500) Channel4::Calc2E(&ch[4], dest[4], dest[5]);
	}
	else 
	{
		fbch = 3;
		if (activech & 0x005) Channel4::Calc2(&ch[0], dest[0], dest[1]);
		if (activech & 0x050) Channel4::Calc2(&ch[2], dest[2], dest[3]);
		if (activech & 0x500) Channel4::Calc2(&ch[4], dest[4], dest[5]);
	}
#elif defined(FM_USE_CALC2)
	if (activech & 0x005) Channel4::Calc2(&ch[0], dest[0], dest[1]);
	if (activech & 0x050) Channel4::Calc2(&ch[2], dest[2], dest[3]);
