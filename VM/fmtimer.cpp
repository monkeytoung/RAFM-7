...
//	タイマー制御
//
void FASTCALL Timer::SetTimerControl(uint data)
{
	uint tmp = regtc ^ data;
	regtc = uint8(data);
...
//	タイマーA 周期設定
//
void FASTCALL Timer::SetTimerA(uint addr, uint data)
{
	uint tmp;
...
//	タイマーB 周期設定
//
void FASTCALL Timer::SetTimerB(uint data)
{
...
//	タイマー時間処理
//
bool FASTCALL Timer::Count(int32 us)
{
	bool event = false;
...
//
int32 FASTCALL Timer::GetNextEvent()
{
...
//	タイマー基準値設定
//
void FASTCALL Timer::SetTimerBase(uint clock)
{
